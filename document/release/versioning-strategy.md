...
- workflow 负责执行机械动作：计算版本号、打 tag、创建 GitHub Release、触发 Docker 发布。
...
| Docker 发布触发 | `v*.*.*` tag | 与手动 release workflow 天然衔接 |
...
关键点：`.github/workflows/docker-publish.yml` 已经在 `v*.*.*` tag push 时发布 Docker 镜像。因此 release workflow 只要推送正式 tag，就能自然触发 Docker 发布。
...
| `create_github_release` | boolean | 默认 `true`，是否创建 GitHub Release |
...
维护者手动触发
  ...
  |-- push tag
  |-- 创建 GitHub Release
  |-- 由 docker-publish.yml 自动响应 tag 并发布 Docker 镜像
```

建议 tag 使用 annotated tag：

```bash
git tag -a "v1.1.0" -m "IMX-Forge v1.1.0"
```
...
由于正式 tag 默认创建 GitHub Release，README badge 与 Docker tag、GitHub Release 不会出现明显版本分裂。
...
| # | 项目 | 类型 | 说明 |
|---|---|---|---|
| 1 | `.github/workflows/release.yml` | 新增 | 手动触发 release，计算版本号、打 tag、创建 Release |
...
| 5 | `.github/workflows/docker-publish.yml` | 复用 | 继续由 `v*.*.*` tag 自动触发 Docker 发布 |
