...
| Docker 发布触发 | `v*.*.*` tag + tag message 策略 | 与手动 release workflow 衔接，并支持按版本跳过 |
...
关键点：`.github/workflows/docker-publish.yml` 已经在 `v*.*.*` tag push 时响应 Docker 发布。release workflow 会在 annotated tag message 中写入 `Docker-Image: true/false`，Docker workflow 据此决定是否实际构建镜像。
...
维护者手动触发
  ...
  |-- 按 publish_docker_image 决定 docker-publish.yml 是否发布 Docker 镜像
```
...
| 5 | `.github/workflows/docker-publish.yml` | 修改 | 继续响应 `v*.*.*` tag，并按 tag message 中的 `Docker-Image` 决定是否发布 |
